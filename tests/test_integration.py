...
@pytest.mark.skipif(
    sys.platform == "win32",
    reason="TODO: Windows path normalization issue - test expects RUNNER~1 but Windows normalizes to runneradmin in output",
)
def test_doctor_shows_default_repo_setting(git_repo):
    """Doctor command should display default_repo configuration.

    TODO: Fix for Windows - the test creates paths with short names (RUNNER~1)
    but Windows normalizes them to long names (runneradmin) in command output,
    causing the assertion to fail. Need Windows access to properly handle
    path normalization in the assertion.
    """
    repo = git_repo["repo"]
    fake_home = git_repo["fake_home"]
...
def test_completion_function_handles_missing_default_repo_gracefully(monkeypatch):
...
def test_completion_function_handles_invalid_default_repo_gracefully(monkeypatch):
...
def test_completion_function_filters_origin_prefix(monkeypatch):
